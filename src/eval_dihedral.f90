	...
	do ib4 = 1, 64
		...
		do j = 1, ictmolecule
			...
			do i = 1, 18
				...
				if(arg.le.-1.0D0) arg = -0.99999999999D0
				if(arg.ge.1.0D0) arg = 0.99999999999D0
	      
				thijkn = acos(arg)
				costh = cos(thijkn)
				sinth = sin(thijkn)
	      
				r1 = -dva(i) + 2.0D0 * dvb(i) * sin(2.0D0 * thijkn) / sinth - 3.0D0 * dvc(i) * sin(3.0D0 * thijkn) / sinth
