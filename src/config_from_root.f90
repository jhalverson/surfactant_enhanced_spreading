	subroutine config_from_root(iaq, ibq, icq)
	...
	do j = 1, 64
		...
		do i = 1, ictw1(ib216)
			...
			do k = 1, 3
				...
				m = (i - 1) * 3 + k
				iatomw = iatomw + 1
				xt(iatomw) = rxw1(m, ib216)
				yt(iatomw) = ryw1(m, ib216)
				zt(iatomw) = rzw1(m, ib216)
	...
	do j = 1, 64
		...
		do i = 1, ictw2(ib512)
			...
			do k = 1, 3
				...
				m = (i - 1) * 3 + k
				iatomw = iatomw + 1
				xt(iatomw) = rxw2(m, ib512)
				yt(iatomw) = ryw2(m, ib512)
				zt(iatomw) = rzw2(m, ib512)
	...
	do j = 1, 64
		...
		do i = 1, icts(ib1000)
			...
			do k = 1, 29
				...
				m = (i - 1) * 29 + k
				iatoms = iatoms + 1
				xsurf(iatoms) = rxs(m, ib1000)
				ysurf(iatoms) = rys(m, ib1000)
				zsurf(iatoms) = rzs(m, ib1000)
	...
	call write_pdb(xt, yt, zt, iwtrmle, xsurf, ysurf, zsurf, isrfmle, icq)
